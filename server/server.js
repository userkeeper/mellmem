// server.js — v3: + favorites + sorting
...
async function initDb() {
  await pool.query(`
    ...
    CREATE INDEX IF NOT EXISTS idx_footage_downloads ON footage(downloads DESC);
    CREATE INDEX IF NOT EXISTS idx_footage_created ON footage(created_at DESC);

    CREATE TABLE IF NOT EXISTS favorites (
      user_tg_id BIGINT REFERENCES users(tg_id) ON DELETE CASCADE,
      footage_id INTEGER REFERENCES footage(id) ON DELETE CASCADE,
      created_at TIMESTAMP DEFAULT NOW(),
      PRIMARY KEY (user_tg_id, footage_id)
    );
    CREATE INDEX IF NOT EXISTS idx_fav_user ON favorites(user_tg_id);
    CREATE INDEX IF NOT EXISTS idx_fav_footage ON favorites(footage_id);
  ...
  // Миграция: добавить favorites_count если его нет
  await pool.query(`
    ALTER TABLE footage ADD COLUMN IF NOT EXISTS favorites_count INTEGER DEFAULT 0;
  `);
...
function validateTelegramInitData(initData) {
  ...
  try {
    ...
    params.delete('hash');
    const arr = [];
    for (const [k, v] of [...params.entries()].sort((a, b) => a[0].localeCompare(b[0]))) arr.push(`${k}=${v}`);
    const secretKey = createHmac('sha256', 'WebAppData').update(BOT_TOKEN).digest();
    const calcHash = createHmac('sha256', secretKey).update(arr.join('\n')).digest('hex');
...
async function upsertUser(u) {
  await pool.query(`
    ...
    ON CONFLICT (tg_id) DO UPDATE SET
      ...
      photo_url = EXCLUDED.photo_url, last_seen = NOW()
  `, [u.id, u.username || null, u.first_name || null, u.photo_url || null]);
}

async function requireTgUser(req, res, next) {
  const u = validateTelegramInitData(req.headers['x-tg-init-data']);
  if (!u) return res.status(401).json({ error: 'invalid telegram auth' });
  const { rows } = await pool.query('SELECT is_banned FROM users WHERE tg_id = $1', [u.id]);
...
app.post('/api/admin/login', (req, res) => {
  ...
  res.json({ token: jwt.sign({ role: 'admin' }, JWT_SECRET, { expiresIn: '7d' }) });
});

app.get('/api/admin/check', requireAdmin, (req, res) => res.json({ ok: true }));

// ========= PUBLIC =========
// /api/footage?sort=new|popular|favorites&category=slug
app.get('/api/footage', async (req, res) => {
  ...
  if (user) {
    const { rows } = await pool.query('SELECT footage_id FROM favorites WHERE user_tg_id = $1', [user.id]);
    rows.forEach(r => myFavIds.add(r.footage_id));
  }

  let orderBy = 'f.created_at DESC';
  if (sort === 'popular') orderBy = 'f.downloads DESC, f.created_at DESC';
  if (sort === 'favorites') orderBy = 'f.favorites_count DESC, f.created_at DESC';

  const { rows } = await pool.query(`
    SELECT f.id, f.title, f.category_slug, f.tags, f.duration, f.downloads, f.favorites_count,
           f.video_key, f.preview_key, f.source_url, f.created_at,
  ...
  res.json(rows.map(r => ({
    ...
    downloads: r.downloads, favorites_count: r.favorites_count || 0,
    is_favorite: myFavIds.has(r.id),
    ...
    uploader: r.uploader_id ? { id: r.uploader_id, username: r.uploader_username, name: r.uploader_name } : null
  })));
});
...
// ========= FAVORITES =========
app.post('/api/favorite/:id', requireTgUser, async (req, res) => {
  const id = parseInt(req.params.id);
  // toggle
  const { rows } = await pool.query(
    'SELECT 1 FROM favorites WHERE user_tg_id = $1 AND footage_id = $2',
    [req.tgUser.id, id]);

  if (rows.length) {
    await pool.query('DELETE FROM favorites WHERE user_tg_id = $1 AND footage_id = $2',
      [req.tgUser.id, id]);
    await pool.query('UPDATE footage SET favorites_count = GREATEST(favorites_count - 1, 0) WHERE id = $1', [id]);
    res.json({ is_favorite: false });
  } else {
    await pool.query('INSERT INTO favorites (user_tg_id, footage_id) VALUES ($1, $2) ON CONFLICT DO NOTHING',
      [req.tgUser.id, id]);
    await pool.query('UPDATE footage SET favorites_count = favorites_count + 1 WHERE id = $1', [id]);
    res.json({ is_favorite: true });
  }
});

app.get('/api/my-favorites', requireTgUser, async (req, res) => {
  const { rows } = await pool.query(`
    SELECT f.id, f.title, f.category_slug, f.tags, f.duration, f.downloads, f.favorites_count,
           f.video_key, f.preview_key, f.source_url, f.created_at,
           c.name AS category_name,
           u.tg_id AS uploader_id, u.username AS uploader_username, u.first_name AS uploader_name
    FROM favorites fav
    JOIN footage f ON f.id = fav.footage_id
    LEFT JOIN categories c ON c.slug = f.category_slug
    LEFT JOIN users u ON u.tg_id = f.uploader_tg_id
    WHERE fav.user_tg_id = $1 AND f.status = 'approved'
  ...
  res.json(rows.map(r => ({
    id: r.id, title: r.title, category_slug: r.category_slug, category_name: r.category_name,
    tags: r.tags || [], duration: r.duration ? parseFloat(r.duration) : null,
    downloads: r.downloads, favorites_count: r.favorites_count || 0, is_favorite: true,
    video_url: `${R2_PUBLIC_URL}/${r.video_key}`,
    preview_url: r.preview_key ? `${R2_PUBLIC_URL}/${r.preview_key}` : null,
    source_url: r.source_url, created_at: r.created_at,
    uploader: r.uploader_id ? { id: r.uploader_id, username: r.uploader_username, name: r.uploader_name } : null
...
app.post('/api/download/:id', async (req, res) => {
  ...
  const user = validateTelegramInitData(req.headers['x-tg-init-data']);
  const { rows } = await pool.query(
    `UPDATE footage SET downloads = downloads + 1 WHERE id = $1 AND status = 'approved'
     RETURNING video_key, title, downloads, uploader_tg_id`, [id]);
  if (!rows.length) return res.status(404).json({ error: 'not found' });
  const row = rows[0];
  if (row.uploader_tg_id) {
    await pool.query('UPDATE users SET total_downloads_received = total_downloads_received + 1 WHERE tg_id = $1',
      [row.uploader_tg_id]).catch(() => {});
  }
  ...
  res.json({ url: `${R2_PUBLIC_URL}/${row.video_key}`, filename: `${safeName}.mp4`, downloads: row.downloads });
});
...
async function checkUploadLimit(tgId) {
  const { rows } = await pool.query(
    `SELECT COUNT(*)::int AS cnt FROM footage WHERE uploader_tg_id = $1 AND created_at > NOW() - INTERVAL '1 hour'`,
    [tgId]);
  return rows[0].cnt < 5;
}
...
app.post('/api/user/upload', requireTgUser, userUpload.single('video'), async (req, res) => {
  ...
  try {
    ...
    const { rows } = await pool.query(`
      INSERT INTO footage (title, category_slug, tags, video_key, preview_key, duration, size_bytes, source_url, uploader_tg_id, status)
    ...
    await pool.query('UPDATE users SET uploads_count = uploads_count + 1 WHERE tg_id = $1', [req.tgUser.id]);

    fs.unlinkSync(tmpPath); fs.unlinkSync(tmpPreview);
    res.json({ id: rows[0].id, status: 'pending', message: 'Отправлено на модерацию' });
  } catch (e) {
...
app.get('/api/user/me', requireTgUser, async (req, res) => {
  ...
  res.json({
    ...
    footage: myFootage.map(f => ({
      ...f, preview_url: f.preview_key ? `${R2_PUBLIC_URL}/${f.preview_key}` : null
    }))
  });
});
...
app.post('/api/admin/upload', requireAdmin, adminUpload.single('video'), async (req, res) => {
  ...
  try {
    ...
    const { rows } = await pool.query(`
      INSERT INTO footage (title, category_slug, tags, video_key, preview_key, duration, size_bytes, source_url, status)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'approved') RETURNING id
    `, [title, category_slug || null, tagsArr, videoKey, previewKey, duration, req.file.size, source_url || null]);
...
app.get('/api/admin/pending', requireAdmin, async (req, res) => {
  const { rows } = await pool.query(`
    SELECT f.*, u.username AS uploader_username, u.first_name AS uploader_name, c.name AS category_name
    FROM footage f LEFT JOIN users u ON u.tg_id = f.uploader_tg_id
    LEFT JOIN categories c ON c.slug = f.category_slug
...
app.post('/api/admin/moderate/:id', requireAdmin, async (req, res) => {
  ...
  if (action === 'approve') {
    ...
    const { rows } = await pool.query(
      `UPDATE footage SET status = 'rejected' WHERE id = $1 RETURNING video_key, preview_key, uploader_tg_id`, [id]);
    if (rows.length) {
      await deleteFromR2(rows[0].video_key);
      await deleteFromR2(rows[0].preview_key);
      if (rows[0].uploader_tg_id) {
        await pool.query('UPDATE users SET uploads_count = GREATEST(uploads_count - 1, 0) WHERE tg_id = $1',
...
app.post('/api/admin/categories', requireAdmin, async (req, res) => {
  ...
  await pool.query('INSERT INTO categories (slug, name) VALUES ($1, $2) ON CONFLICT (slug) DO UPDATE SET name = $2',
    [slug, name]);
  res.json({ ok: true });
});
